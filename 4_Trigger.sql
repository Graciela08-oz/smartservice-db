...
CREATE OR REPLACE FUNCTION public.fn_trg_cliente()
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_cliente();


-- 2. INTEGRIDAD: Evitar que el stock disponible sea menor a cero
CREATE OR REPLACE FUNCTION public.fn_trg_stock()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_stock();


-- 3. TRAZABILIDAD / KARDEX: Registrar ajustes manuales de stock en Kardex
CREATE OR REPLACE FUNCTION public.fn_trg_kardex()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_kardex();


-- 4. AUDITORÍA DE PROCESO: Guardar cambio de estado de orden de trabajo en bitácora
CREATE OR REPLACE FUNCTION public.fn_trg_orden()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_orden();


-- 5. NORMALIZACIÓN DE DATOS: Convertir correo de usuario a minúsculas
CREATE OR REPLACE FUNCTION public.fn_trg_usuario()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_usuario();


-- 6. AUTOMATIZACIÓN OPERATIVA: Actualización de stock al registrar consumo de material
CREATE OR REPLACE FUNCTION public.fn_trg_consumo_material()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_trg_consumo_material();


-- 7. CÁLCULO FINANCIERO: Asignación automática del monto total a la Nota de Servicio desde la Cotización
CREATE OR REPLACE FUNCTION public.fn_actualizar_monto_nota_servicio()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
...
FOR EACH ROW
EXECUTE FUNCTION public.fn_actualizar_monto_nota_servicio();