...
CREATE OR REPLACE PROCEDURE public.sp_registrar_cliente_completo(
...
BEGIN
    ...
    IF p_nom_sistema IS NOT NULL THEN
        INSERT INTO public.sistema_instalado (
            orden_trabajo_id, establecimiento_id, nombre_sistema, numero_serie, 
            fecha_instalacion, fecha_proximo_mantenimiento
        ) VALUES (
            NULL, v_est_id, p_nom_sistema, p_num_serie, 
...
CREATE OR REPLACE PROCEDURE public.sp_crear_cotizacion(
...
BEGIN
    INSERT INTO public.cotizacion (
        ...
        fecha_emision, fecha_vencimiento, estado, total
    ) VALUES (
        p_establecimiento_id, p_usuario_id, p_numero_cotizacion, 
        p_fecha_emision, p_fecha_vencimiento, 'PENDIENTE', 0.00
    ...
    IF p_servicios_json IS NOT NULL THEN
        ...
        LOOP
            INSERT INTO public.detalle_cotizacion_servicio (
                ...
                v_cotizacion_id,
                (v_item->>'servicio_id')::BIGINT,
                (v_item->>'horas_estimadas')::DECIMAL(12,2),
                (v_item->>'precio_hora')::DECIMAL(12,2)
            );
        END LOOP;
    END IF;
END;
...
CREATE OR REPLACE PROCEDURE public.sp_aprobar_cotizacion_y_generar_orden(
...
CREATE OR REPLACE PROCEDURE public.sp_registrar_entrada_stock(
...
CREATE OR REPLACE PROCEDURE public.sp_cerrar_orden_y_emitir_nota(
    p_orden_trabajo_id BIGINT,
    p_numero_nota VARCHAR,
    p_anticipo_pagado DECIMAL(12,2),
    p_observacion TEXT
)
LANGUAGE plpgsql AS $$
BEGIN
    ...
    INSERT INTO public.nota_servicio (
        ...
        monto_total, anticipo_pagado, observacion, estado
    ) VALUES (
        p_orden_trabajo_id, p_numero_nota, NOW(), 
        0.00, p_anticipo_pagado, p_observacion, 'EMITIDA'
    );
END;
$$;